...
resource "aws_ecr_repository" "grant_app" {
...
resource "aws_ecr_repository_policy" "grant_app_lambda_pull" {
  repository = aws_ecr_repository.grant_app.name

  policy = jsonencode({
    Version = "2008-10-17",
    Statement = [
      # Allow Lambda to pull (optional for frontend)
      {
        Sid = "AllowLambdaPull",
        Effect = "Allow",
        Principal = {
